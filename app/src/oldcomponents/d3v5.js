...
export class Timeline extends Component {
    componentDidMount(){
        ...
        //var xScale = d3.scaleOrdinal().rangeBands([0,dim.graphWidth],0.05);
        let xScale = d3.scaleBand()
                    .domain([0,dim.graphWidth])
                    .range([0.05]);
        var xLocalScale = d3.scaleOrdinal();
        //var yScale = d3.scaleOrdinal().rangePoints([dim.graphHeight, 0]);
        let yScale = d3.scalePoint()
            .range([dim.graphHeight, 0]);
        var colorScale = d3.scaleOrdinal(d3.schemeCategory10);
        //var inputScale = d3.scaleOrdinal().rangeBands([0,dim.width-margin.right]);
        let inputScale = d3.scalePoint()
            .range([0,dim.width-margin.right]);
        ...
        xAxisObj.call(xAxis);
        yAxisObj.call(yAxis);
        //xLocalScale.rangeBands([0,xScale]).domain(d3.range(barWidth));
        //var xScale = d3.scaleOrdinal().rangeBands([0,dim.graphWidth],0.05);
        xLocalScale = d3.scaleBand()
                    .domain([0,xScale])
                    .range([barWidth]);
        ...
        buttons.append('text')
            ...
            .attr('x',function(d){return inputScale/2;})
            .attr('y',0)
        ...
        var votes = graphLayer.selectAll('.vote').data(displaydata).enter().append('circle')
            .attr('class','vote')
            .attr('r',radius)
            .attr('cx',(d) => {console.log(d) ;if(d[time].label !== null) {
                console.log(newxScale(d[time].idx))
            } 
